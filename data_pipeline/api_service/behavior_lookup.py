...
WINDOW_SIZE = int(os.getenv('WINDOW_SIZE', 1))  # 默认 window=1

# 预加载所有excluded_users_behavior
base_dir = os.path.dirname(os.path.abspath(__file__))

# 加载行为CSV路径（支持本地/服务器环境）
...
print(f"加载行为数据 from {behavior_csv_path} ...")
...
print(f"行为数据加载完成，共有 {len(user_behavior)} 个用户")
...
def get_user_histories(target_ts, max_history=50, window_size=WINDOW_SIZE):
    ...
    return result  # 返回 dict[user_id] = [item_id1, item_id2, ...]