...
MODEL_PREDICTION_COUNT = Counter('model_prediction_total', 'Total number of model predictions', ['model'])
MODEL_PREDICTION_ERROR = Counter('model_prediction_error_total', 'Total number of model errors', ['model'])
MODEL_PREDICTION_LATENCY = Histogram('model_prediction_seconds', 'Time spent on model prediction', ['model'])
RECOMMENDATION_ITEMS_COUNT = Histogram('recommendation_items_count', 'Number of items in recommendations')
REDIS_KEYS_GAUGE = Gauge('redis_keys_count', 'Number of keys in Redis', ['type'])
MODEL_READY_GAUGE = Gauge('model_ready', 'Whether the model is ready', ['model'])
...
async def process_user(user_id):
    ...
    if user_history:
        ...
        try:
            ...
            MODEL_PREDICTION_COUNT.labels(model='recommendation_system').inc()
            MODEL_PREDICTION_LATENCY.labels(model='recommendation_system').observe(time.time() - start_time)
            
            # Track recommendation count
            RECOMMENDATION_ITEMS_COUNT.observe(len(recommendations))
            ...
            MODEL_PREDICTION_ERROR.labels(model='recommendation_system').inc()
...
async def continuous_monitoring():
    ...
    while True:
        try:
            ...
            REDIS_KEYS_GAUGE.labels(type='all').set(len(redis_client.keys("*")))
            REDIS_KEYS_GAUGE.labels(type='user_profiles').set(len(redis_client.keys("user_profile:*")))
            REDIS_KEYS_GAUGE.labels(type='recommendations').set(len(redis_client.keys("recommendations:*")))
            ...
            MODEL_PREDICTION_ERROR.labels(model='monitoring').inc()
...
async def startup_event():
    ...
    # Set model ready gauges
    MODEL_READY_GAUGE.labels(model='lightgcn').set(1 if recommendation_model.lightgcn.loaded else 0)
    MODEL_READY_GAUGE.labels(model='din').set(1 if recommendation_model.din.loaded else 0)
    MODEL_READY_GAUGE.labels(model='ranknet').set(1 if recommendation_model.ranknet.loaded else 0)
